...
public:
    struct node {
        ...
        int cnt, y;
        node *l, *r, *p;
        node(T value, int cnt, int y) : value(value), aug(value), cnt(cnt), y(y), l(nullptr), r(nullptr), p(nullptr) {}
    };
private:
    Op op;
    T e;
...
public:
    explicit aug_treap(T e, int seed = std::chrono::steady_clock::now().time_since_epoch().count()) : op(Op()), e(e), rng(seed) {}
