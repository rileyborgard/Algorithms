...
public:
    ...
    BitVector(std::vector<uint64_t> v) : _a(std::move(v)) { _n = v.size() * 64; }
    ...
    BitVector operator|(const BitVector &b) const {
        BitVector out(std::max(_n, b._n));
        size_t m = std::min(_a.size(), b._a.size());
        for (size_t k = 0; k < m; k++) {
            out._a[k] = _a[k] | b._a[k];
        }
        if (_a.size() > m) {
            std::copy(_a.begin() + m, _a.end(), out._a.begin() + m);
        } else if (b._a.size() > m) {
            std::copy(b._a.begin() + m, b._a.end(), out._a.begin() + m);
        }
        return out;
    }
    BitVector operator&(const BitVector &b) const {
